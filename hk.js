const puppeteer=require ('puppeteer');
const loginLink='https://www.hackerrank.com/auth/login';
const email='ashish.free@mail.com';
const password='Ashish@free02';
const codeObj=require('./codes');

(async function(){
    try{
        let browserInstance=await puppeteer.launch({          //promise
            headless:false,
            args:['--start-maximized'],
            defaultViewport:null
        }) 

        let newTab=await browserInstance.newPage()
        await newTab.goto(loginLink)
        await newTab.type("input[id='input-1']",email,{delay:50})
        await newTab.type("input[type='password']",password,{delay:50})
        await newTab.click('button[data-analytics="LoginPassword"]', {delay:50})
        await waitAndClick('.topic-card a[data-attr1="algorithms"]',newTab)
        await waitAndClick('input[value="warmup"]',newTab)
        let questionArr= await newTab.$$('.ui-btn.ui-btn-normal.primary-cta.ui-btn-line-primary.ui-btn-styled',{delay:50})
        let questionWillBeSolvedPromise=questionSolver(newTab,questionArr[0],codeObj.answers[0])
        return questionWillBeSolvedPromise
    }
    catch(error){
        console.log(error)
    }
})()

async function waitAndClick(selector,cPage){
    await cPage.waitForSelector(selector)
    let selectorClicked=cPage.click(selector)
    return selectorClicked
}

async function questionSolver(page,question,answer){
    try{
    await question.click();
    await waitAndClick('.monaco-editor.no-user-select.vs',page)
    await waitAndClick('.checkbox-input',page)
    await page.waitForSelector('textarea.custominput',page)
    await page.type('textarea.custominput',answer,{delay:5})
    page.keyboard.down('Control')
    page.keyboard.press('A',{delay:100})
    page.keyboard.press('X',{delay:100})
    page.keyboard.up('Control')
    await waitAndClick('.monaco-editor.no-user-select.vs',page)
    await page.keyboard.down('Control')
    await page.keyboard.press('A',{delay:100})
    await page.keyboard.press('V',{delay:100})
    await page.keyboard.up('Control')
    await page.click('.hr-monaco__run-code',{delay:50})
    let pageisclicked=page.click('.hr-monaco__run-code',{delay:50})
    return pageisclicked
    }
    catch(error){
        console.log(error)
    }
}